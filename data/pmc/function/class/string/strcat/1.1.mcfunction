...
data modify storage pmc:io stack[-1].str1 set string entity 504D43-44-0-0-1 text 4 -2
data modify storage pmc:io stack[-1].str_tmp.a set from storage pmc:io stack[-1].PARAM.source[1]
data modify entity 504D43-44-0-0-1 text set value {storage: "pmc:io", nbt: "stack[-1].str_tmp"}
data modify storage pmc:io stack[-1].str2 set string entity 504D43-44-0-0-1 text 4 -2

data modify storage pmc:io stack[-1].sep set from storage pmc:io stack[-1].PARAM.sep[0]
execute if score #__len__ pmc.var matches 2.. run function pmc:class/string/strcat/1.1.2

function pmc:class/string/strcat/1.1.1 with storage pmc:io stack[-1]
